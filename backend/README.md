...
| `/submit-result` | none | Body `{date: "YYYY-MM-DD", attempts: [[1-4, 1-4, 1-4, 1-4], ...]}` — anonymous, analytics-only ping sent once a puzzle finishes (see `fourplay/results.py`). Each attempt is the difficulty of the 4 tiles guessed together (all 4 equal = a correct guess of that difficulty; otherwise a wrong guess). Computes a score (+difficulty per correct guess, -1 per wrong guess) and a success/failure verdict server-side and **only logs them** — nothing is written to DynamoDB, and neither value is ever returned to the client. |
...
Each run:
1. Queries constructed puzzles where `used` is unset/false. If none are available, logs it and does nothing else — admins need to keep the queue stocked.
2. Picks the **oldest** one (FIFO by `createdAt`) — first constructed, first published, so nothing sits in the queue indefinitely while newer ones keep getting picked instead.
3. Assigns the puzzle's `id`: lists the `puzzles/` prefix in the bucket, finds the most recently published dated file (`puzzles/YYYY-MM-DD.json` — the `default00N.json` fallback files, see below, don't match that pattern and are ignored automatically), reads its `id`, and adds 1. If no dated file exists yet, starts at `1` (the `default00N.json` files all carry `id: 0`).
4. Writes `puzzles/{YYYY-MM-DD}.json` (tomorrow's date) to the frontend's S3 bucket: `{"id": ..., "date": "...", "groups": [...]}`, its 4 groups sorted by difficulty ascending.
5. Marks the chosen puzzle `used: true` + `puzzleDate`, and stamps that same `puzzleDate` onto its 4 group records too (their `used_in_puzzle` is already `true` from construction time — this just records *when* they actually ran, for the admin's own reference).
