...
3. Takes the **most recent** missing date (not oldest — a deliberate choice; see the comment on `_most_recent_missing_date()` in `lambda_function.py` for what that means for a multi-day backlog).
...
7. Re-lists `blog/*`, rebuilds `sitemap.xml` (home page, `submit-group.html`, and one `<url>` per dated post with its S3 `LastModified` as `<lastmod>`), and writes it to the bucket root — this step always runs, even on the no-op path in step 2, so the sitemap self-heals regardless of whether a new post was generated this run.
